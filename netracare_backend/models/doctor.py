...
class Doctor(db.Model):
    ...
    availability_schedule = db.Column(
        db.Text
    )  # JSON: {"monday": ["09:00-12:00", "14:00-17:00"], ...}
    ...
    consultations = db.relationship(
        "Consultation", back_populates="doctor", lazy="dynamic"
    )

    def get_availability_schedule(self) -> dict:
        """Get availability schedule as dictionary"""
        return (
            json.loads(self.availability_schedule) if self.availability_schedule else {}
        )
    ...
    def to_public_dict(self) -> dict:
        ...
        return {
            ...
            "image": self.profile_image_url
            or f"https://i.pravatar.cc/150?img={self.id}",
...
class DoctorPatient(db.Model):
    ...
    patient = db.relationship(
        "User", backref=db.backref("assigned_doctors", lazy="dynamic")
    )
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "last_consultation": (
                self.last_consultation.isoformat() if self.last_consultation else None
            ),
            "last_test_date": (
    ...
    def get_patient_summary(self) -> dict:
        ...
        va = (
            VisualAcuityTest.query.filter_by(user_id=pid)
            .order_by(VisualAcuityTest.created_at.desc())
            .first()
        )
        if va:
            score = (
                round((va.correct_answers / va.total_questions) * 100)
                if va.total_questions
                else 0
            )
            scores.append(score)
            if va.created_at:
        ...
        cv = (
            ColourVisionTest.query.filter_by(user_id=pid)
            .order_by(ColourVisionTest.created_at.desc())
            .first()
        )
        if cv:
            scores.append(cv.score or 0)
            if cv.created_at and (
                latest_test_date is None or cv.created_at > latest_test_date
            ):
                latest_test_date = cv.created_at

        bf = (
            BlinkFatigueTest.query.filter_by(user_id=pid)
            .order_by(BlinkFatigueTest.created_at.desc())
            .first()
        )
        if bf:
            alertness = (
                round((1 - bf.drowsy_probability) * 100)
                if bf.drowsy_probability is not None
                else 100
            )
            scores.append(alertness)
            if bf.created_at and (
                latest_test_date is None or bf.created_at > latest_test_date
            ):
                latest_test_date = bf.created_at

        pr = (
            PupilReflexTest.query.filter_by(user_id=pid)
        ...
        if pr:
            ...
            if pr.created_at and (
                latest_test_date is None or pr.created_at > latest_test_date
            ):
                latest_test_date = pr.created_at

        et = (
            EyeTrackingTest.query.filter_by(user_id=pid)
        ...
        if et and et.overall_performance_score is not None:
            ...
            if et.created_at and (
                latest_test_date is None or et.created_at > latest_test_date
            ):
        ...
        try:
            if (
                computed_score != self.health_score
                or latest_test_date != self.last_test_date
            ):
        ...
        return {
            ...
            "lastTestDate": (
                latest_test_date.isoformat()
                if latest_test_date
                else (self.last_test_date.isoformat() if self.last_test_date else None)
            ),
            "profileImageUrl": (
                patient.profile_image_url
                if hasattr(patient, "profile_image_url")
                else None
            ),
        }
