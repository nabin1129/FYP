...
class DatabaseMigration:
    ...
    def backup_recommendation(self):
        ...
        backup_path = (
            self.db_path + f'.backup_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
        )
        self.log("=" * 70, "INFO")
        ...
        response = (
            input("\nDo you want to continue with migration? (yes/no): ")
            .strip()
            .lower()
        )
    ...
    def migrate_user_table(self):
        ...
        try:
            ...
            self.log(
                f"Existing columns in 'user' table: {len(existing_columns)}", "INFO"
            )
            ...
            for column_name, column_type in new_columns.items():
                if column_name in existing_columns:
                    ...
                    self.log(
                        f"Column '{column_name}' already exists - skipping", "WARNING"
                    )
                else:
                    try:
                        ...
                        self.log(
                            f"[OK] Added column: {column_name} ({column_type})",
                            "SUCCESS",
                        )
