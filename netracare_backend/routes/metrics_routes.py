...
# API Models
accuracy_model = metrics_ns.model('ModelAccuracy', {
...
test_stats_model = metrics_ns.model('TestStats', {
    ...
    'tests_completed_7d': fields.Integer(description='Tests completed in last 7 days'),
    'tests_completed_30d': fields.Integer(description='Tests completed in last 30 days'),
    'average_tests_per_user': fields.Float(description='Average tests per user'),
    'timestamp': fields.String(description='Query timestamp')
})

notification_stats_model = metrics_ns.model('NotificationStats', {
    ...
    'average_read_time_hours': fields.Float(description='Average time to read (hours)'),
...
class BlinkAccuracy(Resource):
    ...
    def get(self, current_user):
        ...
        try:
            ...
            false_positives = sum(1 for t in tests if t.prediction == 'drowsy' and not t.alert_triggered)
            
            # False negatives: predicted alert but user reported no issue (approximation)
            false_negatives = sum(1 for t in tests if t.prediction == 'notdrowsy' and t.alert_triggered)
...
class TestStatistics(Resource):
    ...
    def get(self, current_user):
        ...
        try:
            ...
            active_7d = db.session.query(func.count(func.distinct(BlinkFatigueTest.user_id))) \
                .filter(BlinkFatigueTest.created_at >= week_ago).scalar() or 0
            active_7d += db.session.query(func.count(func.distinct(EyeTrackingTest.user_id))) \
                .filter(EyeTrackingTest.created_at >= week_ago).scalar() or 0
            
            active_30d = db.session.query(func.count(func.distinct(BlinkFatigueTest.user_id))) \
                .filter(BlinkFatigueTest.created_at >= month_ago).scalar() or 0
            active_30d += db.session.query(func.count(func.distinct(EyeTrackingTest.user_id))) \
                .filter(EyeTrackingTest.created_at >= month_ago).scalar() or 0
            
            # Tests completed
            tests_7d = BlinkFatigueTest.query.filter(BlinkFatigueTest.created_at >= week_ago).count() + \
                       EyeTrackingTest.query.filter(EyeTrackingTest.created_at >= week_ago).count()
            
            tests_30d = BlinkFatigueTest.query.filter(BlinkFatigueTest.created_at >= month_ago).count() + \
                        EyeTrackingTest.query.filter(EyeTrackingTest.created_at >= month_ago).count()
