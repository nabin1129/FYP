...
reminder_model = notification_ns.model(
    "ReminderCreate",
    {
        ...
        "scheduled_for": fields.String(
            description="Optional ISO datetime for follow-up context"
        ),
    },
)
    ...
    def get(self, current_user):
        ...
        try:
            ...
            unread_count = Notification.query.filter_by(
                recipient_type="user", recipient_id=current_user.id, is_read=False
            ).count()
...
class DoctorNotifications(Resource):
    ...
    def get(self, current_doctor):
        ...
        try:
            ...
            query = Notification.query.filter_by(
                recipient_type="doctor", recipient_id=current_doctor.id
            )
            ...
            unread_count = Notification.query.filter_by(
                recipient_type="doctor", recipient_id=current_doctor.id, is_read=False
            ).count()
...
class MarkDoctorNotificationRead(Resource):
    ...
    def post(self, current_doctor, notification_id):
        ...
        try:
            ...
            if (
                notification.recipient_type != "doctor"
                or notification.recipient_id != current_doctor.id
            ):
                return {"message": "Not authorized"}, 403
