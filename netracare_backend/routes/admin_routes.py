...
user_update_model = admin_ns.model(
    "AdminUserUpdate",
    {
...
admin_login_model = admin_ns.model(
    "AdminLogin",
    {
...
analytics_query_model = admin_ns.model(
    "AdminAnalyticsQuery",
...
class AdminLogin(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            logging.info(
                "[ADMIN] Login attempt received for email=%s", email or "<empty>"
            )
            ...
            if not is_valid:
                logging.warning(
                    "[ADMIN] Login rejected: invalid password for email=%s", email
                )
                return {"message": "Invalid admin credentials"}, 401

            token = generate_admin_token(BaseConfig.ADMIN_EMAIL)
            logging.info(
                "[ADMIN] Login successful for email=%s", BaseConfig.ADMIN_EMAIL
            )
            return {
    ...
    def get(self, current_admin=None):
        ...
        try:
            ...
            demographics = {
                ...
                "age_buckets": {
                    "under_18": User.query.filter(
                        User.age.isnot(None), User.age < 18
                    ).count(),
                    "18_to_29": User.query.filter(
                        User.age >= 18, User.age <= 29
                    ).count(),
                    "30_to_44": User.query.filter(
                        User.age >= 30, User.age <= 44
                    ).count(),
                    "45_to_59": User.query.filter(
                        User.age >= 45, User.age <= 59
                    ).count(),
            ...
            usage = {
                "window_days": days,
                "eye_tracking_tests": EyeTrackingTest.query.filter(
                    EyeTrackingTest.created_at >= cutoff
                ).count(),
                "visual_acuity_tests": VisualAcuityTest.query.filter(
                    VisualAcuityTest.created_at >= cutoff
                ).count(),
                "colour_vision_tests": ColourVisionTest.query.filter(
                    ColourVisionTest.created_at >= cutoff
                ).count(),
                "blink_fatigue_tests": BlinkFatigueTest.query.filter(
                    BlinkFatigueTest.created_at >= cutoff
                ).count(),
                "pupil_reflex_tests": PupilReflexTest.query.filter(
...
screening_campaign_model = admin_ns.model(
    "ScreeningCampaign",
    {
        "next_date": fields.String(
            required=True, description="Scheduled screening date (YYYY-MM-DD)"
        ),
        "target_users": fields.String(
            description="Filter: all, inactive_30d, inactive_60d, or comma-separated user IDs"
        ),
    },
)
...
class ScreeningReminderCampaign(Resource):
    ...
    def post(self, current_admin=None):
        ...
        try:
            ...
            if target == "all":
                ...
                users = User.query.filter(
                    (
                        ...
                        .filter(EyeTrackingTest.created_at >= cutoff)
                        .count()
                        == 0
                    )
            ...
            logging.info(
                "[ADMIN] Sent screening reminders to %d users for date %s",
                count,
                next_date,
            )
            return {
...
def _user_to_admin_dict(user: User, include_history: bool = False) -> dict:
    ...
    latest_visual = (
        VisualAcuityTest.query.filter_by(user_id=user.id)
        .order_by(VisualAcuityTest.created_at.desc())
        .first()
    )
    latest_colour = (
        ColourVisionTest.query.filter_by(user_id=user.id)
        .order_by(ColourVisionTest.created_at.desc())
        .first()
    )
    latest_blink = (
        BlinkFatigueTest.query.filter_by(user_id=user.id)
        .order_by(BlinkFatigueTest.created_at.desc())
        .first()
    )
    latest_pupil = (
        PupilReflexTest.query.filter_by(user_id=user.id)
        .order_by(PupilReflexTest.created_at.desc())
        .first()
    )
    ...
    latest_dates = [
        t.created_at
        for t in [latest_eye, latest_visual, latest_colour, latest_blink, latest_pupil]
    ...
    response = {
        ...
        "recent_tests": {
            "eye_tracking": {
                "created_at": (
                    latest_eye.created_at.isoformat()
                    if latest_eye and latest_eye.created_at
                    else None
                ),
                "gaze_accuracy": latest_eye.gaze_accuracy if latest_eye else None,
                "classification": (
                    latest_eye.performance_classification if latest_eye else None
                ),
                "duration": latest_eye.test_duration if latest_eye else None,
                "fixation_stability": (
                    latest_eye.fixation_stability_score if latest_eye else None
                ),
                "saccade_consistency": (
                    latest_eye.saccade_consistency_score if latest_eye else None
                ),
            },
            "visual_acuity": {
                "created_at": (
                    latest_visual.created_at.isoformat()
                    if latest_visual and latest_visual.created_at
                    else None
                ),
                ...
                "score": (
                    ...
                    if latest_visual and latest_visual.total_questions
                    else None
                ),
            ...
            "colour_vision": {
                "created_at": (
                    ...
                    if latest_colour and latest_colour.created_at
                    else None
                ),
            ...
            "blink_fatigue": {
                "created_at": (
                    ...
                    if latest_blink and latest_blink.created_at
                    else None
                ),
                ...
                "alertness_percentage": (
                    round((1 - latest_blink.drowsy_probability) * 100)
                    if latest_blink and latest_blink.drowsy_probability is not None
                    else None
                ),
                "avg_blinks_per_minute": (
                    latest_blink.avg_blinks_per_minute if latest_blink else None
                ),
            ...
            "pupil_reflex": {
                "created_at": (
                    ...
                    if latest_pupil and latest_pupil.created_at
                    else None
                ),
                "nystagmus_detected": (
                    latest_pupil.nystagmus_detected if latest_pupil else None
                ),
                "nystagmus_severity": (
                    latest_pupil.nystagmus_severity if latest_pupil else None
                ),
                "reaction_time": latest_pupil.reaction_time if latest_pupil else None,
                "constriction_amplitude": (
                    latest_pupil.constriction_amplitude if latest_pupil else None
                ),
    ...
    if include_history:
        response["test_history"] = {
            "eye_tracking": [
                t.to_dict()
                for t in EyeTrackingTest.query.filter_by(user_id=user.id)
                .order_by(EyeTrackingTest.created_at.desc())
                .all()
            ],
            "visual_acuity": [
                t.to_dict()
                for t in VisualAcuityTest.query.filter_by(user_id=user.id)
                .order_by(VisualAcuityTest.created_at.desc())
                .all()
            ],
            "colour_vision": [
                t.to_dict()
                for t in ColourVisionTest.query.filter_by(user_id=user.id)
                .order_by(ColourVisionTest.created_at.desc())
                .all()
            ],
            "blink_fatigue": [
                t.to_dict()
                for t in BlinkFatigueTest.query.filter_by(user_id=user.id)
                .order_by(BlinkFatigueTest.created_at.desc())
                .all()
            ],
            "pupil_reflex": [
                t.to_dict()
                for t in PupilReflexTest.query.filter_by(user_id=user.id)
                .order_by(PupilReflexTest.created_at.desc())
                .all()
            ],
        }
