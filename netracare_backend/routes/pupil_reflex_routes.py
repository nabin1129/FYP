...
# Create namespace
pupil_reflex_ns = Namespace(
    "pupil-reflex", description="Pupil Reflex and Nystagmus Detection Tests"
)

# API Models
start_test_model = pupil_reflex_ns.model(
    "StartPupilReflexTest",
    {
        "test_type": fields.String(
            required=True, description="Type of test: pupil_reflex or nystagmus"
        ),
        "eye_tested": fields.String(
            required=True, description="Eye being tested: left, right, or both"
        ),
    },
)

analyze_video_model = pupil_reflex_ns.model(
    "AnalyzeVideo",
    {
        "test_id": fields.String(required=True, description="Test session ID"),
        "flash_timestamps": fields.List(
            fields.Float, description="Timestamps when flash was triggered (in seconds)"
        ),
    },
)

test_result_model = pupil_reflex_ns.model(
    "PupilReflexResult",
    {
        ...
        "pupil_response_time_ms": fields.Float(
            description="Pupil response time in milliseconds"
        ),
        "pupil_constriction_percent": fields.Float(
            description="Pupil constriction percentage"
        ),
        "nystagmus_detected": fields.Boolean(
            description="Whether nystagmus was detected"
        ),
        "nystagmus_type": fields.String(
            description="Type of nystagmus: horizontal, vertical, rotary, or mixed"
        ),
        "nystagmus_severity": fields.String(
...
def calculate_pupil_response(video_path, flash_timestamps):
    ...
    try:
        ...
        for i in range(
            flash_frame,
            min(flash_frame + int(fps), int(cap.get(cv2.CAP_PROP_FRAME_COUNT))),
        ):
            cap.set(cv2.CAP_PROP_POS_FRAMES, i)
            ret, frame = cap.read()
            if ret:
...
def detect_nystagmus_movements(video_path):
    ...
    try:
        ...
        if h_has_nystagmus and v_has_nystagmus:
            movement_type = "mixed"
            confidence = (
                min(h_std / (h_std + v_std), v_std / (h_std + v_std)) * 0.8 + 0.2
            )
...
def generate_diagnosis(
    ...
    if pupil_response_time is not None and constriction_percent is not None:
        if pupil_response_time > 300:
            ...
            recommendations.append(
                "Consult neurologist for potential nerve damage assessment"
            )

        if constriction_percent < 20:
            issues.append("Weak pupil constriction (<20%)")
            recommendations.append(
                "Consider ophthalmology consultation for pupil function evaluation"
            )
        elif constriction_percent > 80:
            issues.append("Excessive pupil constriction (>80%)")
            recommendations.append(
                "Monitor for photophobia or light sensitivity issues"
            )
    ...
    if nystagmus_detected:
        ...
        if severity == "severe":
            ...
            recommendations.append(
                "Consider neurological evaluation to rule out vestibular disorders"
            )
    ...
    def post(self, current_user):
        ...
        try:
            ...
            filename = secure_filename(
                f"{test_id}_{uuid.uuid4().hex}.{video_file.filename.rsplit('.', 1)[1].lower()}"
            )
            ...
            try:
                future = _analysis_executor.submit(
                    _analyse, video_path, flash_timestamps
                )
            ...
            response_payload = {
                ...
                "results": {
                    ...
                    "clinical_output_version": serialized_test.get(
                        "clinical_output_version"
                    ),
...
class TestHistory(Resource):
    ...
    def get(self, current_user):
        ...
        try:
                ...
                results.append(
                    {
                        ...
                        "diagnosis": test.diagnosis,
                        "clinical_output_version": serialized.get(
                            "clinical_output_version"
                        ),
            ...
            return {"total_tests": len(results), "tests": results}, 200
...
class SimpleTestSubmission(Resource):
    ...
    def post(self, current_user):
        ...
        try:
            ...
            nystagmus_detected = (
                request.form.get("nystagmus_detected", "").lower() == "true"
            )
            ...
            if not recommendations:
                ...
                if constriction_amplitude == "Weak":
                    recommendations_list.append(
                        "Consult ophthalmologist for pupil function assessment"
                    )
                if symmetry == "Unequal":
                    recommendations_list.append(
                        "Immediate medical attention recommended for asymmetric response"
                    )
                if nystagmus_detected:
                    recommendations_list.append(
                        "Ophthalmologist consultation for nystagmus evaluation"
                    )
                if not recommendations_list:
                    recommendations_list.append(
                        "Pupil reflex appears normal - continue routine eye care"
                    )
            ...
            if "image" in request.files:
                ...
                if image_file.filename != "":
                    filename = secure_filename(
                        f"{current_user.id}_{uuid.uuid4().hex}.jpg"
                    )
...
class GetAllTests(Resource):
    ...
    def get(self, current_user):
        ...
        try:
            ...
            reaction_times = [
                t.reaction_time for t in tests if t.reaction_time is not None
            ]
            avg_reaction_time = (
