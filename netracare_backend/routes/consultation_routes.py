...
book_consultation_model = consultation_ns.model(
    "BookConsultation",
    {
        ...
        "preferred_datetime": fields.String(
            description="Preferred date/time ISO format"
        ),
        "doctor_slot_id": fields.Integer(
            description="Doctor slot ID (required for physical)"
        ),
    },
)

schedule_consultation_model = consultation_ns.model(
    "ScheduleConsultation",
    {
        "scheduled_at": fields.String(
            required=True, description="Scheduled datetime ISO format"
        ),
    },
)

update_consultation_model = consultation_ns.model(
    "UpdateConsultation",
    {
...
send_message_model = consultation_ns.model(
    "SendMessage",
    {
...
doctor_slot_model = consultation_ns.model(
    "DoctorSlot",
    {
        "slot_start_at": fields.String(
            required=True, description="Slot start datetime in ISO UTC format"
        ),
    ...
    def post(self, current_user):
        ...
        try:
            ...
            if consultation_type == "physical":
                ...
                if not slot_id:
                    return {
                        "message": "doctor_slot_id is required for physical consultation"
                    }, 400
            ...
            patient_name = (
                current_user.name if hasattr(current_user, "name") else "A patient"
            )
            if consultation_type == "physical":
            ...
            existing_link = DoctorPatient.query.filter_by(
                doctor_id=doctor_id, patient_id=current_user.id
            ).first()

            if not existing_link:
                link = DoctorPatient(
                    doctor_id=doctor_id, patient_id=current_user.id, status="active"
                )
                db.session.add(link)
        ...
        try:
            consultations = (
                Consultation.query.filter_by(
                    doctor_id=current_doctor.id, status="pending"
                )
    ...
    def get(self, current_doctor):
        ...
        try:
            ...
            query = Consultation.query.filter_by(
                doctor_id=current_doctor.id, status="scheduled"
            )
...
class ConsultationDetail(Resource):
    ...
    def get(self, current_user, consultation_id):
        ...
        try:
            ...
            return {"consultation": consultation.to_dict(include_messages=True)}, 200
    ...
    def post(self, current_doctor, consultation_id):
        ...
        try:
            ...
            if consultation.consultation_type == "physical":
                return {
                    "message": "Physical consultations are scheduled from doctor slots"
                }, 409
            ...
            if not is_future_utc(scheduled_at):
                return {
                    "message": "Cannot schedule consultation in the past. Please select a future date and time."
                }, 400
    ...
    def post(self, current_doctor, consultation_id):
        ...
        try:
            ...
            if data.get("follow_up_date"):
                consultation.follow_up_date = datetime.fromisoformat(
                    data["follow_up_date"]
                )
            ...
            link = DoctorPatient.query.filter_by(
                doctor_id=current_doctor.id, patient_id=consultation.patient_id
            ).first()
            if link:
        ...
        try:
            ...
            if first_attachment:
                message_type = (
                    ...
                    or first_attachment.get("attachment_type")
                    or message_type
                    or "file"
                ).lower()
            ...
            if first_attachment:
                ...
                if not test_type:
                    test_type = first_attachment.get(
                        "linked_entity_title"
                    ) or first_attachment.get("linkedEntityTitle")
                if test_id is None:
                    linked_id = first_attachment.get(
                        "linked_entity_id"
                    ) or first_attachment.get("linkedEntityId")
            ...
            # Create notification for doctor
            patient_name = (
                current_user.name if hasattr(current_user, "name") else "Patient"
            )
...
class ShareTestResult(Resource):
    ...
    def post(self, current_user, consultation_id):
        ...
        try:
            ...
            return {"message": "Test result shared", "data": message.to_dict()}, 201
...
class DoctorAssignedRecords(Resource):
    ...
    def get(self, current_user):
        ...
        try:
            ...
            messages = (
                ConsultationMessage.query.join(
            ...
            for msg in messages:
                ...
                message_type_lower = (
                    msg.message_type.lower() if msg.message_type else ""
                )
                ...
                record = {
                    "id": msg.id,
                    "title": msg.content
                    or f'{msg.test_type or "Document"} from {doctor_name}',
                ...
                if is_clinical:
                    ...
                    if msg.test_type:
                        ...
                        if "scan" in test_type_lower or "image" in test_type_lower:
                            ...
                            "prescription" in test_type_lower
                            or "medicine" in test_type_lower
                        ):
