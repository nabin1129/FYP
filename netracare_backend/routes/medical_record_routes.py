...
medical_records_ns = Namespace(
    "medical-records", description="Medical record management"
)

medical_record_model = medical_records_ns.model(
    "MedicalRecord",
    {
        ...
        "category": fields.String(
            description="general, follow_up, diagnosis, treatment, referral"
        ),
...
class DoctorMedicalRecordDetail(Resource):
    ...
    def put(self, current_doctor, record_id):
        ...
        if not record or record.doctor_id != current_doctor.id:
            return {"message": "Medical record not found"}, 404
        try:
            updated = _service().update_record(
                record, updated_by_id=current_doctor.id, data=request.get_json() or {}
            )
            return {
    ...
    def delete(self, current_doctor, record_id):
        ...
        if not record or record.doctor_id != current_doctor.id:
            return {"message": "Medical record not found"}, 404
        try:
            deleted = _service().soft_delete_record(
                record, deleted_by_id=current_doctor.id
            )
            return {
                "message": "Medical record deleted successfully",
                "record": deleted.to_dict(),
            }, 200
        except Exception as exc:
            db.session.rollback()
...
class PatientMedicalRecordList(Resource):
    ...
    def get(self, current_user):
        ...
        for consultation in consultations:
            doctor_name = (
                consultation.doctor.name if consultation.doctor else "Unknown Doctor"
            )
...
class AdminMedicalRecordDetail(Resource):
    ...
    def put(self, current_admin=None, record_id=None):
        ...
        try:
            updated = _service().update_record(
                record, updated_by_id=record.doctor_id, data=request.get_json() or {}
            )
            return {
                "message": "Medical record updated successfully",
                "record": updated.to_dict(),
    ...
    def delete(self, current_admin=None, record_id=None):
        ...
        try:
            deleted = _service().soft_delete_record(
                record, deleted_by_id=record.doctor_id
            )
            return {
                "message": "Medical record deleted successfully",
                "record": deleted.to_dict(),
...
class AdminMedicalRecordReassign(Resource):
    ...
    def post(self, current_admin=None, record_id=None):
        ...
        try:
            ...
            db.session.commit()
            return {
                "message": "Medical record reassigned successfully",
                "record": record.to_dict(),
            }, 200
        except Exception as exc:
            db.session.rollback()
...
class AdminMedicalRecordRestore(Resource):
    ...
    def post(self, current_admin=None, record_id=None):
        ...
        if not record:
            return {"message": "Medical record not found"}, 404
        try:
            restored = _service().restore_record(
                record, restored_by_id=record.doctor_id
            )
            return {
                "message": "Medical record restored successfully",
                "record": restored.to_dict(),
            }, 200
        except Exception as exc:
            db.session.rollback()
