...
doctor_register_model = doctor_ns.model(
    "DoctorRegister",
    {
        ...
        "nhpc_number": fields.String(
            required=True, description="NHPC registration number"
        ),
        "qualification": fields.String(
            required=True, description="Medical qualification"
        ),
...
doctor_login_model = doctor_ns.model(
    "DoctorLogin",
    {
...
doctor_update_model = doctor_ns.model(
    "DoctorUpdate",
    {
...
link_patient_model = doctor_ns.model(
    "LinkPatient",
    {
...
change_password_model = doctor_ns.model(
    ...
    def post(self):
        ...
        try:
            ...
            if not doctor or not check_password_hash(
                doctor.password_hash, data["password"]
            ):
...
class DoctorProfile(Resource):
    ...
    def get(self, current_doctor):
        """Get doctor profile"""
        return {"doctor": current_doctor.to_dict(include_sensitive=True)}, 200
...
class DoctorPatients(Resource):
    ...
    def get(self, current_doctor):
        ...
        try:
            ...
            return {"patients": patients, "total": len(patients)}, 200
    ...
    def post(self, current_doctor):
        ...
        try:
            ...
            existing = DoctorPatient.query.filter_by(
                doctor_id=current_doctor.id, patient_id=patient_id
            ).first()
...
class DoctorPatientDetail(Resource):
    ...
    def get(self, current_doctor, patient_id):
        ...
        try:
            ...
            test_history = {
                "visual_acuity": [
                    t.to_dict()
                    for t in VisualAcuityTest.query.filter_by(user_id=patient_id)
                ...
                "colour_vision": [
                    t.to_dict()
                    for t in ColourVisionTest.query.filter_by(user_id=patient_id)
                ...
                "blink_fatigue": [
                    t.to_dict()
                    for t in BlinkFatigueTest.query.filter_by(user_id=patient_id)
                ...
                "pupil_reflex": [
                    t.to_dict()
                    for t in PupilReflexTest.query.filter_by(user_id=patient_id)
                ...
                "eye_tracking": [
                    t.to_dict()
                    for t in EyeTrackingTest.query.filter_by(user_id=patient_id)
    ...
    def put(self, current_doctor, patient_id):
        ...
        try:
            ...
            return {"message": "Patient info updated", "link": link.to_dict()}, 200
...
class AdminDoctorManage(Resource):
    ...
    def put(self, doctor_id):
        ...
        try:
            ...
            if data.get("nhpc_number"):
                doctor.nhpc_number = data["nhpc_number"]
            if data.get("qualification"):
                doctor.qualification = data["qualification"]
            if data.get("experience_years") is not None:
                doctor.experience_years = data["experience_years"]
            if data.get("working_place") is not None:
                doctor.working_place = data["working_place"]
