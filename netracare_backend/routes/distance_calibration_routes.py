...
# API Models
calibration_model = calibration_ns.model(
    "Calibration",
    {
        "calibration_method": fields.String(
            required=True, description="arcore, arkit, manual, credit_card"
        ),
        "platform": fields.String(description="android or ios"),
        "measured_distance_cm": fields.Float(
            required=True, description="Measured distance in cm"
        ),
        "target_distance_cm": fields.Float(
            default=100.0, description="Target distance (default 100cm)"
        ),
    ...
    def post(self, current_user):
        ...
        try:
            ...
            calibration = DistanceCalibration(
                ...
                + timedelta(days=30),  # Calibration valid for 30 days
            )
