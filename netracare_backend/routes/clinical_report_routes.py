...
create_model = clinical_report_ns.model(
    ...
    {
        'ai_summary': fields.String(required=True, description='AI-generated summary text'),
...
review_model = clinical_report_ns.model(
    'ReviewClinicalReport',
    {
        'action': fields.String(
            required=True, description='validated or rejected', enum=['validated', 'rejected']
        ),
...
class ClinicalReportReview(Resource):
    ...
    def patch(self, current_user, report_id):
        ...
        report.reviewed_at = datetime.now(timezone.utc).replace(tzinfo=None, microsecond=0)
        db.session.commit()
