...
profile_update_model = auth_ns.model(
    "ProfileUpdate",
    {
        "full_name": fields.String(description="Full name"),
        ...
        try:
            ...
            if "date_of_birth" in data:
                current_user.age = (
                    ...
                    - datetime.strptime(data["date_of_birth"], "%Y-%m-%d").year
                )
            if "gender" in data:
            ...
            if (
                "eye_disease_details" in data
                or "family_history" in data
                or "current_medications" in data
            ):
...
class TestHistory(Resource):
    ...
    def get(self, current_user):
        ...
        try:
            history = {
                "visual_acuity_tests": [
                    test.to_dict() for test in current_user.visual_acuity_tests.all()
                ],
                "colour_vision_tests": [
                    test.to_dict() for test in current_user.colour_vision_tests.all()
                ],
                "pupil_reflex_tests": [
                    test.to_dict() for test in current_user.pupil_reflex_tests.all()
                ],
                "blink_fatigue_tests": [
                    test.to_dict() for test in current_user.blink_fatigue_tests.all()
                ],
                "eye_tracking_sessions": [
                    test.to_dict() for test in current_user.eye_tracking_sessions.all()
                ],
                "ai_reports": [
                    report.to_dict() for report in current_user.ai_reports.all()
                ],
            }
