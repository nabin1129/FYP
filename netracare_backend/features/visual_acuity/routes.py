...
from features.visual_acuity.model import (
    validate,
    calculate_logmar,
    logmar_to_snellen,
    classify_severity,
...
# Create namespace
visual_acuity_ns = Namespace(
    "visual-acuity", description="Visual acuity test operations"
)

# API Models for documentation
test_submission_model = visual_acuity_ns.model(
    "VisualAcuityTestSubmission",
    {
        "correct_answers": fields.Integer(
            required=True, description="Number of correct answers"
        ),
        "total_questions": fields.Integer(
            required=True, description="Total number of questions"
        ),
        "test_variant": fields.String(
            required=False,
            description="Test variant: snellen, tumbling_e, or landolt_c",
        ),
    },
)

test_response_model = visual_acuity_ns.model(
    "VisualAcuityTestResponse",
    {
    ...
    def get(self, current_user):
        ...
        try:
            ...
            return {
                ...
                "total": VisualAcuityTest.query.filter_by(
                    user_id=current_user.id
                ).count(),
                "limit": limit,
                "offset": offset,
...
class VisualAcuityStatistics(Resource):
    ...
    def get(current_user, self):
        ...
        try:
            ...
            severity_counts = {}
            for test in tests:
                severity_counts[test.severity] = (
                    severity_counts.get(test.severity, 0) + 1
                )
