...
# Create namespace
camera_eye_tracking_ns = Namespace(
    "camera-eye-tracking", description="Camera-based eye tracking operations"
)

# API Models for documentation
session_model = camera_eye_tracking_ns.model(
    "CameraEyeTrackingSession",
    {
        "session_name": fields.String(required=False, description="Session name"),
        "duration_seconds": fields.Float(
            required=True, description="Session duration in seconds"
        ),
        "total_blinks": fields.Integer(
            required=True, description="Total blinks detected"
        ),
        "blink_rate_per_minute": fields.Float(
            required=True, description="Blink rate per minute"
        ),
        "ear_statistics": fields.Raw(required=True, description="EAR statistics"),
        "gaze_distribution": fields.Raw(
            required=True, description="Gaze direction distribution"
        ),
        "total_frames": fields.Integer(
            required=True, description="Total frames processed"
        ),
        "frames_with_face": fields.Integer(
            required=True, description="Frames with face detected"
        ),
        "camera_id": fields.Integer(
            required=False, description="Camera device ID", default=0
        ),
        "ear_threshold": fields.Float(
            required=False, description="EAR threshold used", default=0.21
        ),
...
class CameraEyeTrackingSessions(Resource):
    ...
    def post(current_user, self):
        ...
        try:
            ...
            detection_rate = (
                (frames_with_face / total_frames * 100) if total_frames > 0 else 0
            )
    ...
    def get(current_user, self):
        ...
        try:
            ...
            include_events = (
                request.args.get("include_events", "false").lower() == "true"
            )

            # Get sessions
            sessions = (
                CameraEyeTrackingSession.query.filter_by(user_id=current_user.id)
                .order_by(CameraEyeTrackingSession.created_at.desc())
                .limit(limit)
                .offset(offset)
                .all()
            )
            ...
            return {
                "sessions": [
                    session.to_dict(include_events=include_events)
                    for session in sessions
                ],
                "total": CameraEyeTrackingSession.query.filter_by(
                    user_id=current_user.id
                ).count(),
                "limit": limit,
                "offset": offset,
...
class CameraEyeTrackingStatistics(Resource):
    ...
    def get(current_user, self):
        ...
        try:
            ...
            if not sessions:
                return {"total_sessions": 0, "message": "No sessions found"}, 200
            ...
            avg_blink_rate = (
                sum(
                    s.blink_rate_per_minute for s in sessions if s.blink_rate_per_minute
                )
                / total_sessions
            )
            avg_detection_rate = (
                sum(s.detection_rate for s in sessions if s.detection_rate)
                / total_sessions
            )
            ...
            # Average EAR values
            avg_left_ear = (
                sum(s.left_eye_ear_mean for s in sessions if s.left_eye_ear_mean)
                / total_sessions
            )
            avg_right_ear = (
                sum(s.right_eye_ear_mean for s in sessions if s.right_eye_ear_mean)
                / total_sessions
            )
            avg_overall_ear = (
                sum(s.average_ear_mean for s in sessions if s.average_ear_mean)
                / total_sessions
            )

            # Recent sessions
            recent_sessions = sorted(
                sessions, key=lambda x: x.created_at, reverse=True
            )[:5]
    ...
    def post(current_user, self):
        ...
        try:
            ...
            if len(session_ids) < 2:
                return {
                    "message": "At least 2 session IDs required for comparison"
                }, 400
