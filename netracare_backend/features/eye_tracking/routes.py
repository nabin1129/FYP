...
from features.eye_tracking.model import (
    EyeTrackingMetrics,
    EyeTrackingDataset,
...
data_point_model = eye_tracking_ns.model(
    "EyeTrackingDataPoint",
    {
...
test_submission_model = eye_tracking_ns.model(
    "EyeTrackingTestSubmission",
    {
        "test_name": fields.String(
            required=False, default="Eye Tracking Test", description="Test name"
        ),
        "test_duration": fields.Float(
            required=True, description="Test duration in seconds"
        ),
        "gaze_accuracy": fields.Float(
            required=True, description="Gaze accuracy percentage"
        ),
...
upload_data_model = eye_tracking_ns.model(
    "EyeTrackingUploadData",
    {
        "test_name": fields.String(required=True, description="Test name"),
        "data_points": fields.List(
            fields.Nested(data_point_model),
            required=True,
            description="Eye tracking data points",
        ),
        "test_duration": fields.Float(
            required=True, description="Test duration in seconds"
        ),
...
class EyeTrackingTests(Resource):
    ...
    def post(self, current_user):
        ...
        try:
            ...
            test_record = EyeTrackingTest(
                ...
                overall_performance_score=data.get(
                    "overall_score", data["gaze_accuracy"]
                ),
            ...
            if "pupil_metrics" in data:
                ...
                test_record.set_pupil_metrics(
                    metrics.get("left_pupil", {}), metrics.get("right_pupil", {})
                )
            ...
            notif = Notification.create_result_ready(
                current_user.id, "Eye Tracking", test_record.id
            )
    ...
    def get(self, current_user):
        ...
        try:
            ...
            tests = (
                EyeTrackingTest.query.filter_by(user_id=current_user.id)
            ...
            return {
                ...
                "total": EyeTrackingTest.query.filter_by(
                    user_id=current_user.id
                ).count(),
                "limit": limit,
                "offset": offset,
    ...
    def post(self, current_user):
        ...
        try:
            ...
            try:
                ...
                points_with_target = [
                    p for p in all_points if p.has_target and not p.is_blink
                ]
                if points_with_target:
                    target_positions = [
                        (p.target_x, p.target_y) for p in points_with_target
                    ]
                ...
                fixation_durations = [
                    p.fixation_duration
                    for p in dataset.get_data_points()
                    if p.fixation_duration
                ]
                fixation_stability = (
                    EyeTrackingMetrics.calculate_fixation_stability(fixation_durations)
                ...
                saccade_metrics = (
                    EyeTrackingMetrics.calculate_saccade_metrics(saccade_velocities)
            ...
            test_record.set_pupil_metrics(
                pupil_metrics["left_pupil"], pupil_metrics["right_pupil"]
            )
