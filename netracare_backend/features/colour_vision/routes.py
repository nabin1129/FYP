...
# Create namespace
colour_vision_ns = Namespace(
    "colour-vision", description="Colour vision test operations (Ishihara)"
)
...
def _resolve_dataset_path() -> Path:
    ...
    candidates = [
        ...
        here.parents[2]
        / "ishihara_data_set",  # <workspace>/netracare_backend/ishihara_data_set
        here.parents[1]
        / "ishihara_data_set",  # <workspace>/netracare_backend/features/ishihara_data_set
    ]
...
plate_model = colour_vision_ns.model(
    "IshiharaPlate",
    {
        "id": fields.Integer(required=True, description="Plate index in test sequence"),
        "plate_number": fields.Integer(
            required=True, description="Ishihara plate number (0-9)"
        ),
        "image_path": fields.String(
            required=True, description="URL path to plate image"
        ),
        "image_filename": fields.String(required=True, description="Image filename"),
        "correct_answer": fields.String(
            required=True, description="Correct answer for the plate"
        ),
        "options": fields.List(
            fields.String, required=True, description="Multiple choice options"
        ),
        "description": fields.String(description="Plate description"),
    },
)

plates_response_model = colour_vision_ns.model(
    "PlatesResponse",
    {
...
test_submission_model = colour_vision_ns.model(
    "ColourVisionTestSubmission",
    {
        "plate_ids": fields.List(
            fields.Integer, required=True, description="List of plate numbers shown"
        ),
        "plate_images": fields.List(
            fields.String, required=True, description="List of image filenames used"
        ),
        "user_answers": fields.List(
            fields.String, required=True, description="User's selected answers"
        ),
        "test_duration": fields.Float(description="Test duration in seconds"),
    },
)

test_response_model = colour_vision_ns.model(
    "ColourVisionTestResponse",
    {
        ...
        "plate_images": fields.List(fields.String, description="Plate image filenames"),
        "user_answers": fields.List(
            fields.String, description="User answers for each plate"
        ),
        "correct_answers": fields.List(
            fields.String, description="Correct answers for each plate"
        ),
        ...
        "severity": fields.String(
            description="Classification: Normal, Mild Deficiency, Deficiency Detected"
        ),
        "test_duration": fields.Float(description="Test duration in seconds"),
...
def get_random_plates(count=10):
    ...
    selected_plate_numbers = random.sample(
        available_plates, min(count, len(available_plates))
    )
...
class ColourVisionPlates(Resource):
    ...
    @colour_vision_ns.param(
        "count", "Number of plates to retrieve (default: 5)", type=int
    )
    ...
    def get(self, current_user):
        ...
        try:
            ...
            return {"plates": plates, "total_plates": len(plates)}, 200
    ...
    def post(self, current_user):
        ...
        try:
            ...
            if not validation["is_valid"]:
                colour_vision_ns.abort(
                    400, validation.get("error", "Invalid test data")
                )
