...
def get_plate_metadata(plate_number: int) -> dict:
    ...
    if plate_number not in ISHIHARA_PLATE_METADATA:
        raise ValueError(
            f"Invalid plate number: {plate_number}. Must be between 0 and 9."
        )
...
def validate_answers(plate_ids: list, user_answers: list) -> dict:
    ...
    missed_plate_types = {"red_green": 0, "blue_yellow": 0, "total": 0}
    ...
    if control_plate_failed:
        result["warning"] = (
            "Control plate (Plate 0) was incorrect. Test results may be unreliable."
        )
...
def classify_result(
    score: int, control_plate_failed: bool = False, missed_plate_types: dict = None
) -> str:
    """
