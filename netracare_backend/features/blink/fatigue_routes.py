...
# Create namespace
blink_fatigue_ns = Namespace(
    "blink-fatigue", description="Blink and eye fatigue detection operations"
)
...
prediction_response_model = blink_fatigue_ns.model(
    "BlinkFatiguePrediction",
    {
...
test_result_model = blink_fatigue_ns.model(
    "BlinkFatigueTestResult",
    {
...
test_history_model = blink_fatigue_ns.model(
    "BlinkFatigueHistory",
    {
...
fatigue_config_model = blink_fatigue_ns.model(
    "BlinkFatigueConfig",
    {
        "drowsy_threshold": fields.Float(
            description="Probability threshold used for drowsy classification"
        ),
...
class BlinkFatiguePrediction(Resource):
    ...
    def post(self, current_user):
        ...
        if file_ext not in allowed_extensions:
            blink_fatigue_ns.abort(
                400, f"Invalid file type. Allowed: {allowed_extensions}"
            )
...
class BlinkFatigueHistory(Resource):
    ...
    def get(self, current_user):
        ...
        try:
            ...
            # Fallback for legacy rows without user_id
            if not tests:
                tests = (
                    BlinkFatigueTest.query.filter(BlinkFatigueTest.user_id.is_(None))
                    .order_by(BlinkFatigueTest.created_at.desc())
                    .all()
                )
...
class BlinkFatigueStats(Resource):
    ...
    def get(self, current_user):
        ...
        try:
            tests = (
                BlinkFatigueTest.query.filter_by(user_id=current_user.id)
                .order_by(BlinkFatigueTest.created_at.asc())
                .all()
            )
                        ...
                        if 0.5 < movement_magnitude < 2.0:
                            ...
                            if len(movements) > 0:
                                direction_change = np.dot(movement, prev_movement) / (
                                    ...
                                    * np.linalg.norm(prev_movement)
                                    + 1e-5
                                )
                                if (
                                    direction_change < 0.5
                                ):  # Direction change > 60 degrees
...
enhanced_parser.add_argument(
    "video", location="files", type=FileStorage, required=False
)
enhanced_parser.add_argument(
    "analyze_redness", location="form", type=bool, default=False
)
enhanced_parser.add_argument(
    "analyze_microsaccades", location="form", type=bool, default=False
)
    ...
    def post(self, current_user):
        ...
        try:
            ...
            if request.form.get("analyze_microsaccades", "false").lower() == "true":
                if "video" in request.files:
                    ...
                    if video_file.filename != "":
                        ...
                        (
                            microsaccade_count,
                            microsaccade_frequency,
                            microsaccade_fatigue,
                        ) = analyze_microsaccades(video_path)
            ...
            if len(fatigue_indicators) >= 2:
                ...
                recommendation = (
                    "Immediate rest recommended. Avoid driving or operating machinery."
                )
            ...
            return {
                ...
                "eye_redness": (
                    {"score": redness_score, "is_red": is_red, "level": redness_level}
                ...
                "microsaccades": (
                    ...
                    if microsaccade_count is not None
                    else None
                ),
                "overall_assessment": {
