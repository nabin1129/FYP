...
# Create namespace
blink_detection_ns = Namespace(
    "blink-detection", description="Real-time blink detection operations"
)

# API Models
frame_analysis_model = blink_detection_ns.model(
    "FrameAnalysis",
    {
...
test_submission_model = blink_detection_ns.model(
    "BlinkTestSubmission",
    {
        "blink_count": fields.Integer(
            required=True, description="Total blinks counted"
        ),
        "duration_seconds": fields.Integer(required=True, description="Test duration"),
        "drowsiness_probability": fields.Float(
            required=True, description="Drowsiness probability (0-1)"
        ),
        "confidence_score": fields.Float(
            required=True, description="Confidence score (0-1)"
        ),
        "fatigue_level": fields.String(description="Fatigue classification"),
    },
)

test_response_model = blink_detection_ns.model(
    "BlinkTestResponse",
    {
...
try:
    ...
    print("[OK] MediaPipe FaceLandmarker loaded successfully for blink detection")
except FileNotFoundError:
    print("[WARN] MediaPipe model file 'face_landmarker.task' not found")
    print(
...
try:
    ...
    DLIB_AVAILABLE = True
    print("[OK] dlib loaded successfully")
except Exception as e:
    if not MEDIAPIPE_AVAILABLE:
        print(f"[WARN] dlib not available: {e}")
        print("       Blink detection will use basic fallback method")
    DLIB_AVAILABLE = False
    detector = None
    predictor = None

from features.blink.detector import BlinkDetector
    ...
    def post(self, current_user):
        ...
        try:
            ...
            notif = Notification.create_result_ready(
                current_user.id, "Blink & Fatigue", test.id
            )
