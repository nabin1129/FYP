...
import 'package:netracare/services/blink_detection_service.dart';
import 'package:netracare/services/blink_detection_engine.dart';
...
class _BlinkFatigueCNNTestPageState extends State<BlinkFatigueCNNTestPage> {
  ...
  Future<void> _startTest() async {
    ...
    await _blinkEngine?.start(
      ...
      onError: (error) {
        debugPrint('Blink detection error: $error');
      },
    );
  ...
  Future<void> _captureAndAnalyze() async {
    ...
    if (_cameraController == null ||
        ...
        _isCapturing) {
      if (_isCapturing) {
        debugPrint('Capture already in progress, skipping');
      } else {
        _showError('Camera not ready');
      }
      setState(() {
