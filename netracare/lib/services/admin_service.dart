...
class AdminService {
  ...
  int get totalPatients => _doctors.fold(0, (sum, d) => sum + d.patients);
  ...
  String? addDoctor(AdminDoctor doctor) {
    ...
    if (doctorIdExists(doctor.id))
      return 'Doctor ID "${doctor.id}" already exists';
    if (doctorEmailExists(doctor.email))
      return 'Email "${doctor.email}" is already registered';
    if (doctor.password.length < 6)
