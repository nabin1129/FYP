    ...
    showModalBottomSheet(
      ...
      builder: (_) => _DoctorFormSheet(
        ...
        onSaved: (saved) {
          _refresh();
          if (doctor == null) {
            // New doctor added — show credentials
            _showCredentials(saved);
          }
        },
      ),
    );
...
class _DoctorFormSheetState extends State<_DoctorFormSheet> {
  ...
  void _submit() {
    ...
    String? error;
    if (_isEdit) {
      error = widget.service.updateDoctor(widget.doctor!.id, doctor);
    } else {
      error = widget.service.addDoctor(doctor);
    }
    ...
    Navigator.pop(context);
    widget.onSaved(doctor);

    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(_isEdit ? '${doctor.name} updated' : '${doctor.name} added'),
      behavior: SnackBarBehavior.floating,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
    ));
  }
            ...
            child: SingleChildScrollView(
              ...
              child: Form(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _sectionLabel('Credentials (Set by Admin)'),
                    _buildTextField(
                      ctrl: _idCtrl,
                      label: 'Doctor ID *',
                      hint: 'e.g. DOC-006',
