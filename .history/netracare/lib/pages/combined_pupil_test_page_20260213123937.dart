...
class _CombinedPupilTestPageState extends State<CombinedPupilTestPage> with SingleTickerProviderStateMixin {
              ...
              children: [
                const Row(
                  children: [
                    ...
                    SizedBox(width: 8),
                    Text('Test Instructions', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF1f2937))),
                  ],
                ),
      ...
      children: [
        ...
        AnimatedBuilder(
          ...
          builder: (context, child) {
            return Container(
              color: Colors.white.withOpacity(_flashAnimationController.value * 0.7),
            );
          },
        ),
      ...
      child: Container(
        ...
        child: const Column(
          ...
          children: [
            ...
            SizedBox(height: 20),
            Text('Analyzing results...', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500)),
            SizedBox(height: 8),
            Text('Processing video for nystagmus detection', style: TextStyle(fontSize: 13, color: Colors.grey)),
          ],
        ),
      ),
    );
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: isNystagmusNormal ? [Colors.green.shade400, Colors.green.shade600] : [Colors.orange.shade400, Colors.orange.shade600],
              ),
              borderRadius: BorderRadius.circular(12),
            ),
            child: Column(
              children: [
                Icon(isNystagmusNormal ? Icons.check_circle : Icons.warning, color: Colors.white, size: 48),
                const SizedBox(height: 12),
                Text(
                  isNystagmusNormal ? 'Test Complete' : 'Abnormal Results Detected',
          ...
          _buildResultCard(
            ...
            Icons.flash_on,
            [
              _buildResultRow('Reaction Time', '${(_reactionTime * 1000).toStringAsFixed(0)} ms'),
          ...
          if (_nystagmusDetected != null)
            _buildResultCard(
              ...
              [
                _buildResultRow('Status', _nystagmusDetected! ? 'Detected' : 'Not Detected'),
                if (_nystagmusDetected!) ...[
                  _buildResultRow('Type', _nystagmusType ?? 'N/A'),
                  _buildResultRow('Severity', _nystagmusSeverity ?? 'N/A'),
                  _buildResultRow('Confidence', _confidence != null ? '${(_confidence! * 100).toStringAsFixed(1)}%' : 'N/A'),
                ],
              ],
            ),
          const SizedBox(height: 16),
            ...
            _buildResultCard(
              ...
              [
                Padding(
                  ...
                  child: Text(_diagnosis!, style: const TextStyle(color: Color(0xFF6b7280), height: 1.5)),
                ),
              ],
            ),
          const SizedBox(height: 16),
          ...
          if (_recommendations != null)
            _buildResultCard(
              ...
              [
                Padding(
                  padding: const EdgeInsets.only(top: 8),
                  child: Text(_recommendations!, style: const TextStyle(color: Color(0xFF6b7280), height: 1.5)),
                ),
              ],
            ),
          const SizedBox(height: 24),
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(value, style: const TextStyle(fontWeight: FontWeight.w600, color: Color(0xFF1f2937))),
        ],
      ),
    );
  }
