...
class _BlinkFatigueCNNTestPageState extends State<BlinkFatigueCNNTestPage> {
  ...
  Future<void> _initializeCameras() async {
    try {
      ...
      if (cameras == null || cameras!.isEmpty) {
        if (mounted) {
          setState(() {
            errorMessage = 'No camera found on this device. This test requires a camera.';
          });
        }
        return;
      }
  ...
  Future<void> _startTest() async {
    ...
    _blinkTimer = Timer.periodic(const Duration(seconds: 2), (timer) async {
      ...
      // In production, this would use eye tracking algorithms
      final random = Random();
      if (random.nextDouble() < 0.35) { // ~35% chance per 2s interval = ~10-11 blinks/min
        setState(() {
          blinkCount++;
        });
      }
  ...
  Future<void> _captureAndAnalyze() async {
    ...
    try {
      ...
      final result = await BlinkFatigueService.predictDrowsiness(imageToAnalyze);

      // Store the image file path for later saving
      final directory = await getTemporaryDirectory();
      final savedImagePath = path.join(directory.path, 'blink_test_${DateTime.now().millisecondsSinceEpoch}.jpg');
      await imageToAnalyze.copy(savedImagePath);

      if (mounted) {
        setState(() {
          predictionResult = result;
          predictionResult!['saved_image_path'] = savedImagePath; // Store for later save
          testPhase = 2;
          isProcessing = false;
        });
      }
  ...
  Future<void> _saveResults() async {
    ...
    try {
      ...
      final imagePath = predictionResult!['saved_image_path'] as String?;
      File imageFile;
      
      if (imagePath != null && await File(imagePath).exists()) {
        imageFile = File(imagePath);
      } else {
        // Fallback: create a temporary file if image was lost
        final directory = await getTemporaryDirectory();
      ...
      // Submit test to backend with blink data
            ...
            child: Container(
              ...
              child: Column(
                children: [
                  ...
                  const Text(
                    "The test will run for 90 seconds to accurately measure blink rate and fatigue levels",
                    textAlign: TextAlign.center,
                  ...
                  ElevatedButton.icon(
                    ...
                    label: const Text("Start Test (90s)"),
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(isSaving ? 'Saving...' : (isSaved ? 'Results Saved' : 'Save Results')),
              style: ElevatedButton.styleFrom(
