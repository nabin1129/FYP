...
class _BlinkFatigueTestPageState extends State<BlinkFatigueTestPage> {
  ...
  Future<void> _initializeCamera() async {
    ...
    _cameraController = CameraController(
      selectedCamera,
      ResolutionPreset.medium, // Lower resolution for better emulator performance
      enableAudio: false,
  ...
  void _startTest() {
    ...
    // Test progression (90 seconds for accurate blink detection)
    _testTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      setState(() {
        progress += 1.11; // 100 / 90 seconds ≈ 1.11% per second
      });
    ...
    _blinkTimer = Timer.periodic(const Duration(seconds: 2), (timer) async {
      ...
      if (_cameraController != null && _cameraController!.value.isInitialized) {
        ...
        final random = Random();
        if (random.nextDouble() < 0.3) { // ~30% chance per 2s interval = ~9 blinks/min
          setState(() {
            blinkCount++;
          });
        }
  ...
  Future<void> _detectFinalFatigue() async {
    ...
    if (_cameraController != null && _cameraController!.value.isInitialized) {
      try {
        ...
        final result = await BlinkFatigueService.predictDrowsiness(File(xFile.path));
        ...
        debugPrint('⚠️ Fatigue detection failed: $e');
      ...
      if (_cameraController != null && _cameraController!.value.isInitialized) {
        try {
          ...
          imageFile = await _cameraController!.takePicture().timeout(
            ...
            onTimeout: () {
              debugPrint('⏱️ Camera capture timeout - continuing without image');
