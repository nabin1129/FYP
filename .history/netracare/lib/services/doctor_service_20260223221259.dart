...
class DoctorService {
  ...
  Future<List<Patient>> getPatientsAsync() async {
    try {
      final response = await _apiService.getMyPatients();
      if (response['success'] == true && response['patients'] != null) {
        final patientsData = response['patients'] as List;
        return patientsData.map((p) => Patient.fromJson(p)).toList();
      }
    } catch (e) {
  ...
  Future<Patient?> getPatientByIdAsync(String id) async {
    try {
      final response = await _apiService.getPatientDetails(id);
      if (response['success'] == true && response['patient'] != null) {
  ...
  Future<DoctorAnalytics> getAnalyticsAsync() async {
    try {
      final response = await _apiService.getDoctorStats();
      if (response['success'] == true) {
        return DoctorAnalytics.fromJson(response);
      }
    } catch (e) {
      // Fall through to mock data
    }
  ...
  Future<List<ConsultationRequest>> getConsultationRequestsAsync() async {
    try {
      final response = await _apiService.getDoctorConsultations();
      if (response['success'] == true && response['consultations'] != null) {
        final consultationsData = response['consultations'] as List;
        return consultationsData
            .map((c) => ConsultationRequest.fromJson(c))
            .toList();
      }
    } catch (e) {
      // Fall through to mock data
  ...
  Future<List<ConsultationRequest>> getPendingRequestsAsync() async {
    try {
      final response = await _apiService.getDoctorConsultations(status: 'pending');
      if (response['success'] == true && response['consultations'] != null) {
        final consultationsData = response['consultations'] as List;
        return consultationsData
            .map((c) => ConsultationRequest.fromJson(c))
  ...
  Future<bool> acceptRequestAsync(int consultationId, String scheduledDate) async {
    try {
      final response = await _apiService.scheduleConsultation(
        consultationId,
  ...
  Future<bool> rejectRequestAsync(int consultationId) async {
    try {
      final response = await _apiService.cancelConsultation(consultationId);
      return response['success'] == true;
    } catch (e) {
      return false;
    }
  ...
  Future<List<ChatMessage>> getChatHistoryAsync(int consultationId) async {
    try {
      final response = await _apiService.getConsultationMessages(consultationId);
  ...
  Future<ChatMessage?> sendMessageAsync(int consultationId, String message) async {
    try {
      final response = await _apiService.sendMessage(consultationId, message);
