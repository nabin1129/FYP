...
class DoctorService {
  ...
  List<Patient> getFilteredPatients({
    ...
    if (searchQuery != null && searchQuery.isNotEmpty) {
      ...
      result = result.where((p) => p.name.toLowerCase().contains(lowerQuery)).toList();
    }
  ...
  Map<String, dynamic> getPatientStats() {
    ...
    final goodCount = _patients.where((p) => p.status == HealthStatus.good).length;
    final attentionCount = _patients.where((p) => p.status == HealthStatus.attention).length;
    final criticalCount = _patients.where((p) => p.status == HealthStatus.critical).length;
    
    final avgScore = _patients.isEmpty
        ? 0
        : _patients.map((p) => p.healthScore).reduce((a, b) => a + b) / _patients.length;
