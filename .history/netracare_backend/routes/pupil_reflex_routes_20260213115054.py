...
class SimpleTestSubmission(Resource):
    ...
    def post(self, current_user):
        ...
        try:
            ...
            # Estimate constriction percentage based on amplitude
            constriction_map = {
                'Weak': 15.0,
                'Normal': 50.0,
                'Strong': 75.0
            }
            constriction_percent = constriction_map.get(constriction_amplitude, 50.0)
            
            # Generate basic diagnosis
            diagnosis_parts = []
            ...
            recommendations = []
            if response_time_ms > 300:
                recommendations.append("Consider neurological evaluation")
            if constriction_amplitude == 'Weak':
                recommendations.append("Consult ophthalmologist for pupil function assessment")
            if symmetry == 'Unequal':
                recommendations.append("Immediate medical attention recommended for asymmetric response")
            if not recommendations:
                recommendations.append("Pupil reflex appears normal - continue routine eye care")
            ...
            test = PupilReflexTest(
                ...
                test_date=datetime.utcnow(),
                eye_tested='both',
                pupil_response_time_ms=response_time_ms,
                pupil_constriction_percent=constriction_percent,
                diagnosis=diagnosis,
                recommendations="; ".join(recommendations),
                nystagmus_detected=False,
                nystagmus_type=None,
                nystagmus_severity=None,
                nystagmus_confidence=None
            )
            ...
            if 'image' in request.files:
                ...
                if image_file.filename != '':
                    ...
                    test.video_path = image_path  # Reuse video_path field for image
            ...
            return {
                ...
                'symmetry': symmetry,
                'created_at': test.test_date.isoformat() if test.test_date else None,
                'diagnosis': diagnosis,
                'recommendations': "; ".join(recommendations)
            }, 201
        ...
        try:
            ...
            for test in tests:
                test_list.append({
                    'id': test.id,
                    'user_id': test.user_id,
                    'reaction_time': test.pupil_response_time_ms / 1000 if test.pupil_response_time_ms else None,
                    'constriction_amplitude': 'Normal' if test.pupil_constriction_percent and 20 <= test.pupil_constriction_percent <= 80 else 'Abnormal',
                    'symmetry': 'Equal',  # Default for basic tests
                    'test_duration': 10.0,
                    'created_at': test.test_date.isoformat() if test.test_date else None
                })
                
                if test.pupil_response_time_ms:
                    total_reaction_time += test.pupil_response_time_ms / 1000
                    valid_count += 1
            
            avg_reaction_time = total_reaction_time / valid_count if valid_count > 0 else 0
...
class GetSingleTest(Resource):
    ...
    def delete(self, current_user, test_id):
        ...
        try:
            ...
            # Delete associated image/video file
            if test.video_path and os.path.exists(test.video_path):
                try:
                    os.remove(test.video_path)
                except:
                    pass
